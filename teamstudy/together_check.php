...
// 중복 예약 확인 쿼리를 실행합니다.
$overlapStatement = mysqli_prepare($con, "SELECT COUNT(*) FROM together WHERE date = ? AND seat = ? AND ((st_time <= ? AND end_time > ?) OR (st_time < ? AND end_time > ?) OR (st_time < ? AND end_time <= ?) OR (st_time >= ? AND end_time <= ?))");
